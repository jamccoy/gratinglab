...
Pre-alpha. The scalar and integral solvers, the comparison harness, the convergence
harness and the materials layer work end to end — efficiencies are absolute when a
coating is named. RCWA is contributed and not yet integrated.
...
Tip convolution can be undone where the tip actually reached, with the unrecoverable
region marked rather than smoothed over. **Efficiency** is everything else: the problem
spec, the solvers, the comparison and convergence harnesses, the materials layer, and
the resolving power the measured groove count implies.
...
### From a measured surface to a spectrograph number

The scan carries its own groove count, so resolving power needs nothing retyped:

```python
problem = profile.to_problem(coating="Au")     # n_grooves = the grooves averaged
resolving_power(problem, illumination, wavelength=2.0, order=3).r
```

`R = |m|·N` by the Rayleigh criterion, with `line_profile` giving the finite-*N* line
shape it is derived from. A problem whose `n_grooves` is unset is **refused** rather
than treated as infinite: "R is undefined" and "R is very large" are different answers.
Resolving power degraded by real groove-placement error is named as the successor and
deliberately not approximated by the ideal formula.

### The tip is not the surface

An AFM image is the surface dilated by the tip. `tip_correction='erosion'` inverts that
where the inversion exists — a cone-plus-spherical-apex tip, Villarrubia (1997) — and
reports a **certainty map** for where it does not, because the trough a tip never
entered is not recoverable by any algorithm and a correction that quietly filled it in
would be worse than no correction.

It has already earned its keep: on the one real scan whose depth and facet angle
disagree by 7.6°, erosion with the nominal probe recovers *nothing*, and reproducing
the deficit forward takes an apex worn to ~80 nm — so the nominal tip is excluded as
the explanation
...
- **Scalar (Kirchhoff)** — the solve, efficiency per order with per-order visibility, and
  ...
  looking away.
- **Integral method** — the rigorous backend, with the same panel and the same
  provenance discipline, plus its own boundary condition control (perfect conductivity
  or tabulated optical constants). It is minutes where scalar is milliseconds, so the
  window opens on the fast solver and this tab solves when you ask it to.
...
A tab per registered solver is generated from the registry, so a contributed backend
gets its panel by registering rather than by editing the window.
...
| [`docs/theory/integral.md`](docs/theory/integral.md) | The boundary-integral solver: conical formulation, perfect and finite conductivity |
| [`docs/theory/metrology.md`](docs/theory/metrology.md) | What the AFM pipeline assumes, what it measures, and what it does not — including what tip erosion can and cannot give back |
| [`docs/findings.md`](docs/findings.md) | Empirical results, each with the evidence that established it |
| [`docs/references.md`](docs/references.md) | The literature each part is built from, mapped to the part it serves |
