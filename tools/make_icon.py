...
Renders a 1024 px master via :mod:`gratinglab.gui.icon`, downsamples it with
...
OUTPUT = REPO / "resources" / "GratingLab.icns"
...
def main() -> int:
    ...
    from gratinglab.gui.icon import render

    with tempfile.TemporaryDirectory() as scratch:
        master = render(Path(scratch) / "master.png", size=1024)
        iconset = Path(scratch) / "GratingLab.iconset"
        iconset.mkdir()
