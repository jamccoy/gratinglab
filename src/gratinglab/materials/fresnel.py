...
def reflectivity(
    ...
    *,
    ...
    if polarization == "s":
        ...
        smooth = 0.5 * (np.abs(r_s) ** 2 + np.abs(r_p) ** 2)
    else:
        raise ValueError(
    ...
    if roughness_nm == 0.0 or model == "none":
        return smooth
    return smooth * _roughness_factor(
        k1, k2, roughness_nm, wavelength_nm, model
    )
...
def nevot_croce(
    ...
    r"""Névot–Croce factor, :math:`\left|e^{-2 k_{iz} k_{tz} \sigma^2}\right|`.
    ...
    return np.abs(np.exp(-2.0 * k_iz * k_tz * roughness_nm**2)).astype(np.float64)
